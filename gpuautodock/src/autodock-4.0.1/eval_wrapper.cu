 ...
 */
extern "C" void cuda_alloc_wrapper(int natom, int nnum_individuals, maptype map, NonbondParam * nonbondlist, EnergyTables *etbl, Real *charge, Real *ABScharge, int *type, int *ignore_inter, Boole inc14interact, Boole haveflexresidues)
{
    int memsize = 0;
    int i,j,k,l;
...
//    cudaMemcpy(energiesgpu, energiescpu, sizeof(float) * num_individuals, cudaMemcpyHostToDevice);
